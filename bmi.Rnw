...
qplot(agemos, bmi, data=nwaz, geom=c("path"), alpha=I(1/3), colour=factor(id), 
      main="Line Graph for bmi") +
  theme(legend.position="none")
ggplot(nwaz, aes(agemos, bmi), alpha=I(1/10)) + 
  geom_smooth(aes(group=id, colour=factor(id)), methos="lm", size=0.2, se=F) +
  geom_smooth(aes(group=1), method="gam", formula=y~ns(x,3), se=F, colour="red") +
  theme(legend.position="none") +
