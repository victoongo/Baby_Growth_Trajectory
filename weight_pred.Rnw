...
# final for quad (problems is that u and a plot.sitar does not accept mar speced in the plot itself, only the one in the par, but the dv plot.sitar only takes the mar from within the plot spec)
def.par <- par(no.readonly = TRUE) # save default, for resetting...
...
# final for 3 by 1 (the a plot is not on the same yaxis scale as the b and c plot, making the curve look flatter, can use layout() to 
