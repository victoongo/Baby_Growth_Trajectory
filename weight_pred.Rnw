...
nwaz$educ.f <- factor(nwaz$education_final)
...
# Number of measures by child
# summaryBy()
nwaz_nm <- na.omit(nwaz)
nwaz_nm$agec3.f <- factor(cut(nwaz_nm$agemos, c(0, 6, 12, 25), right=F))
summary(data.frame(table(nwaz_nm$id)))

nwaz_freq <- data.frame(table(nwaz_nm$id,nwaz_nm$agec3.f))
names(nwaz_freq) <- list("id","age_group","freq")
summaryBy(freq~age_group, data=nwaz_freq, FUN=list(mean, median))

# aggdata <- subset(nwaz, select=c(id, race.f))
# aggdata <-aggregate(aggdata, by=list('id'), FUN=mean, na.rm=TRUE)
...
countby(nwaz, id, race.f)

by(nwaz, nwaz$race.f, function(nwaz$id) length(unique(nwaz$id)))

tapply(nwaz$id, nwaz$race.f, function(x) length(unique(x))) # best way to go
tapply(nwaz$id, nwaz$race.f, function(x) length(x)/length(unique(x)))
tapply(nwaz$id, nwaz$bmic5.f, function(x) length(x)/length(unique(x)))
tapply(bmi$id, bmi$race_final, function(x) length(unique(x)))
tapply(bmi$id, bmi$race_final, function(x) length(x)/length(unique(x)))
tapply(bmi$id, bmi$race_final, function(x) length(x)/length(unique(x)))
table(bmi$race_final)

aggdata <- by(nwaz,nwaz$id,tail,n=1)
aggdatadf <- do.call("rbind", as.list(aggdata))
table(aggdatadf$race.f)
aggregate(id~race.f, nwaz, length)
@
...
fmla11 <- "~bmic5.f+sex+gest_weight_gain_kg+GestAge_TotalDays+parity.f+mom_age_delv+race.f+educ.f+gest_dbt.f+preg_ht.f+smoker.f+mat_ht_m"

# fmla11 <- "~BMI_LMP_kgm2+sex+gest_weight_gain_kg+GestAge_TotalDays+parity.f+mom_age_delv+race.f+educ.f+gest_dbt.f+preg_ht.f+smoker.f+mat_ht_m"

fmla21 <- "~mat_gest_wt_gain_c3.f+sex+GestAge_TotalDays+parity.f+mom_age_delv+race.f+educ.f+gest_dbt.f+preg_ht.f+smoker.f+mat_ht_m"
...
m11f.coef <- data.frame(summary(m11)$tTable)
save(m11f.coef, file="m11fcoef.Rda")
save(m11f, file="m11f.Rda")

m21.coef <- summary(m21)$tTable
save(m21.coef, file="m21coef.Rda")
write.csv(m21.coef, file="m21.coef.csv")
save(m21, file="m21.Rda")

m11f.coef <- data.frame(summary(m11f)$tTable)
save(m11f.coef, file="m11coefn5.Rda")
save(m11f, file="m11fn5.Rda")

m11.coef <- data.frame(summary(m11)$tTable)
save(m11.coef, file="m11n4coef.Rda")
write.csv(m11.coef, file="m11.coef.csv")
save(m11, file="m11fn4.Rda")
...
m11f.out <- out.sitar(m11f)
m21.out <- out.sitar(m21)
names(m11f.out) <- c("cov", paste('m11f', names(m11f.out)[-1], sep=""))
names(m21.out) <- c("cov", paste('m21', names(m21.out)[-1], sep=""))
all.out <- merge(m11f.out, m21.out, by="cov", all=T)
order.n <- data.frame(cbind(c(20, 17:19, 8, 13, 14, 15, 6, 7, 21, 22, 11, 12, 10, 1:5, 9, 16), 1:22))
...
all.table <- xtable(all.out)
digits(all.table)[c(2,6)] <- 0
print(all.table, floating=F)
...
plot(m21, y2par=list(col='blue'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
dev.copy(png,file="Plotm21dv.png",width = 640, height = 480); dev.off()

plot(m21, opt='a', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(0, 15), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)")
lines(m21, opt='d', col='red', lwd=2, yfun=exp, xlab="Age in Months", ylab="Weight (kg)")
dev.copy(png,file="Plotm21a.png",width = 640, height = 480); dev.off()

plot(m21, opt='u', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(0, 20), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)")
dev.copy(png,file="Plotm21u.png",width = 640, height = 480); dev.off()


plot(m11f, y2par=list(col='blue'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
dev.copy(png,file="Plotm11dv.png",width = 300, height = 300); dev.off()

plot(m11f, opt='a', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(0, 15), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)")
lines(m11f, opt='d', col='red', lwd=2, yfun=exp, xlab="Age in Months", ylab="Weight (kg)")
dev.copy(png,file="Plotm11a.png",width = 640, height = 480); dev.off()

plot(m11f, opt='u', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(0, 20), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)")
dev.copy(png,file="Plotm11u.png",width = 640, height = 480); dev.off()


par(mfrow = c(2, 2), mar = c(4, 4, 4, 1), oma = c(0, 0, 0, 0))
with(m11f, {
  plot(m11f, opt='u', col=1+as.integer(id) %% 5, ylim=c(2, 18), yfun=exp,
       xlab="Age in Months", ylab="Weight (kg)")
  plot(m11f, opt='a', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(2, 18), yfun=exp,
       xlab="Age in Months", ylab="Weight (kg)")
    lines(m11f, opt='d', col='red', lwd=2, yfun=exp, xlab="Age in Months", ylab="Weight (kg)")
  plot(m11f, y2par=list(col='blue'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
})
dev.copy(png,file="Plotm11.png",width = 960, height = 960); dev.off()


par(mfrow = c(1, 2), mar = c(4, 4, 4, 1), oma = c(0, 0, 0, 0))
with(m11f, {
  plot(m11f, opt='u', col=1+as.integer(id) %% 5, ylim=c(2, 18), yfun=exp,
       xlab="Age in Months", ylab="Weight (kg)", main='a')
  plot(m11f, opt='a', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(2, 18), yfun=exp,
       xlab="Age in Months", ylab="Weight (kg)", main='b')
    lines(m11f, opt='d', col='red', lwd=2, yfun=exp, xlab="Age in Months", ylab="Weight (kg)")
#   plot(m11f, y2par=list(col='blue'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
})
dev.copy(png,file="Plotm11.png",width = 960, height = 480); dev.off()


def.par <- par(no.readonly = TRUE) # save default, for resetting...

nf <- layout(matrix(c(1,2,0,3),2,2,byrow = TRUE), c(2,2), c(2,2), TRUE)
layout.show(nf)
...
plot(m11f, opt='a', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(2, 18), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)")
  lines(m11f, opt='d', col='red', lwd=2, yfun=exp, xlab="Age in Months", ylab="Weight (kg)")
# par(mar = c(4,4,4,1), oma = c(2, 2, 2, 2))
plot(m11f, y2par=list(col='blue'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
dev.copy(png,file="Plotm11.png",width = 960, height = 960); dev.off()

plot(m11f, opt='dv', y2par=list(col='blue'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
plot(nwaz$agemos, y1=nwaz$weight, y2=nwaz$GestAge_TotalDays, type='l', xlab="Age in Months", ylab="Weight (kg)")
par(def.par)

# final (it matters to the legend size how the screen device is sized, it wants to stay the same proportion to
# the print out on screen. so when the screen size is small, the png legends ends up big.
# also, it seems the mar parameter specified in par doesn't control the plot.sitar very well, possilbly due to
# internal specification of mar in plot.sitar function. but the resetting default code seems to help)
def.par <- par(no.readonly = TRUE) # save default, for resetting...
...
plot(m11f, opt='a', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(2, 18), yfun=exp,
  ...
  lines(m11f, opt='d', col='red', lwd=2, yfun=exp, xlab="Age in Months", ylab="Weight (kg)")
# par(mar = c(4,4,4,1), oma = c(2, 2, 2, 2))
plot(m11f, y2par=list(col='blue'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, mar = c(4, 4, 1, 4))
plot(m11f, y2par=list(col='blue'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
dev.copy(png,file="Plotm11.png",width = 960, height = 960); dev.off()

par(def.par)


# create the dv plot with abc parameters for covariates
load('m11f.Rda')

plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.0605375640,b=0.1595475100,c=-0.0649566752))
legend("right",lty=1,col=c("black","blue"),legend=c("Normal BMI","BMI > 35"), cex=0.9, inset=0.05)
dev.copy(png,file="Plotm11bmi.png",width = 640, height = 480); dev.off()

plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.1188336132,b=0.5251127982,c=-0.2815792547))
legend("right",lty=1,col=c("black","blue"),legend=c("No Diabetes","Gestational Diabetes"), cex=0.9, inset=0.05)
dev.copy(png,file="Plotm11diab.png",width = 640, height = 480); dev.off()

par(fig=c(0,0.495,0,1), new=T)
  plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
  plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
  plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
       add=T, abc=c(a=0.0605375640,b=0.1595475100,c=-0.0649566752))
  legend("right",lty=1,col=c("black","blue"),legend=c("Normal BMI","BMI > 35"), cex=0.9, inset=0.05)
  title(main="a", cex=1.5)
par(fig=c(0.505,1,0,1), new=T)
  plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
  plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
  plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
       add=T, abc=c(a=0.1188336132,b=0.5251127982,c=-0.2815792547))
  legend("right",lty=1,col=c("black","blue"),legend=c("No Diabetes","Gestational Diabetes"), cex=0.9, inset=0.05)
  title(main="a", cex=1.5)
dev.copy(png,file="Plotm11bmi_diab.png",width = 960, height = 480); dev.off()

# final
par(mfrow = c(1, 2), oma = c(0, 0, 1, 0))
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, mar = c(4, 4, 0, 5))
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.0605375640,b=0.1595475100,c=-0.0649566752))
legend("right",lty=1,col=c("black","blue"),legend=c("Normal BMI","BMI > 35"), cex=0.9, inset=0.05)
title(main="a", font.main=4, cex.main=1, adj=0, outer=T)

plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, mar = c(4, 5, 0, 4))
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.1188336132,b=0.5251127982,c=-0.2815792547))
legend("right",lty=1,col=c("black","blue"),legend=c("w/o Diabetes","w/ Diabetes"), cex=0.9, inset=0.05)
title(main="b", font.main=4, cex.main=1, adj=0.5, outer=T)
dev.copy(png,file="Plotm11bmi_diab.png",width = 960, height = 480); dev.off()


# final for quad (problems is that u and a plot.sitar does not accept mar speced in the plot itself,
# only the one in the par, but the dv plot.sitar only takes the mar from within the plot spec)
def.par <- par(no.readonly = TRUE) # save default, for resetting...

nf <- layout(matrix(c(1,2,3,4),2,2,byrow = TRUE), c(2,2), c(2,2), TRUE)
layout.show(nf)
par(oma = c(0, 0, 0, 0), mar = c(4, 4, 2, 5))

plot(m11f, opt='u', col=1+as.integer(id) %% 5, ylim=c(2, 18), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)", mar = c(4, 4, 2, 5))
title(main="a", font.main=4, cex.main=1.2, adj=0)

plot(m11f, opt='a', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(2, 18), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)", mar = c(4, 4, 2, 5))
  lines(m11f, opt='d', col='red', lwd=2, yfun=exp, xlab="Age in Months", ylab="Weight (kg)")
title(main="b", font.main=4, cex.main=1.2, adj=0)

plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, mar = c(4, 4, 2, 5))
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.0605375640,b=0.1595475100,c=-0.0649566752))
legend("right",lty=1,col=c("black","blue"),legend=c("Normal BMI","BMI > 35"), cex=0.9, inset=0.05)
title(main="c", font.main=4, cex.main=1.2, adj=0)
...
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.1188336132,b=0.5251127982,c=-0.2815792547))
legend("right",lty=1,col=c("black","blue"),legend=c("w/o Diabetes","w/ Diabetes"), cex=0.9, inset=0.05)
title(main="d", font.main=4, cex.main=1.2, adj=0)
dev.copy(png,file="Plotm11q.png",width = 960, height = 960); dev.off()

par(def.par)


# final for quad (problems is that u and a plots does not accept mar speced in the plot itself, only the one in the par)
# the plot axis can use oma to show ticks whem mar is set to 0
# this version makes gapless 2*2 panel plots with modified sitar package.
library(sitar)
setwd("~/Projects/R_lib/bmi/data")
load('m11f.Rda')
load('nwaz.Rda')

par(mfrow = c(2, 2), oma = c(4, 4, 0, 4), mar=c(0,0,0,0))

plot(m11f, opt='u', col=1+as.integer(id) %% 5, ylim=c(2, 18), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)", axes=F)
axis(2, col = "grey40", col.axis = "grey20", at = seq(2,18,4))
box(col = "grey60")
title(main="a", font.main=4, cex.main=1.2, adj=0, line=-1)

plot(m11f, opt='a', col=1+as.integer(id) %% 5, xlim=xaxsd(), ylim=c(2, 18), yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)", axes=F)
  lines(m11f, opt='d', col='red', lwd=2, yfun=exp, xlab="Age in Months", ylab="Weight (kg)")
box(col = "grey60")
title(main="b", font.main=4, cex.main=1.2, adj=0, line=-1)

plot(m11f, y2par=list(col='black', axes=F), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, mar=c(0,0,0,0), axes=F)
plot(m11f, y2par=list(col='black', axes=F), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue', axes=F), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.0605375640,b=0.1595475100,c=-0.0649566752))
axis(2, col = "grey40", col.axis = "grey20", at = seq(4,12,2))
axis(1, col = "grey40", col.axis = "grey20", at = seq(0,25,5))
box(col = "grey60")
legend("right",lty=1,col=c("black","blue"),legend=c("Normal BMI","BMI > 35"), cex=0.9, inset=0.05)
title(main="c", font.main=4, cex.main=1.2, adj=0, line=-1)

plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, mar=c(0,0,0,0), axes=F, add=F, y2axis=TRUE)
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.1188336132,b=0.5251127982,c=-0.2815792547))
axis(1, col = "grey40", col.axis = "grey20", at = seq(0,25,5))
...
box(col = "grey60")
legend("right",lty=1,col=c("black","blue"),legend=c("w/o Diabetes","w/ Diabetes"), cex=0.9, inset=0.05)
title(main="d", font.main=4, cex.main=1.2, adj=0, line=-1)

mtext("Age in Months", side = 1, outer = TRUE, cex=1, line = 2.8)
mtext("Weight (kg)", side = 2, outer = TRUE, cex=1, line = 2.5)
mtext("Weight (kg) Velocity", side = 4, outer=TRUE, cex=1, line=2.8, adj=0.2)

dev.copy(png,file="Plotm11qn.png",width = 960, height = 960); dev.off()


# final for 3 by 1 (the a plot is not on the same yaxis scale as the b and c plot,
# making the curve look flatter, can use layout() to
# correct this by assigning more height for a plot)
library(sitar)
setwd("~/Projects/R_lib/bmi/data")
load('m11f.Rda')
load('nwaz.Rda')

par(mfrow = c(3, 1), oma = c(0, 0, 0, 0), mar = c(4, 4, 2, 4))

plot(m11f, opt='u', col=1+as.integer(id) %% 5, yfun=exp,
     xlab="Age in Months", ylab="Weight (kg)", mar = c(4, 4, 2, 5))
title(main="a", font.main=4, cex.main=1.2, adj=0)

plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, mar = c(4, 4, 2, 4))
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.0605375640,b=0.1595475100,c=-0.0649566752))
legend("right",lty=1,col=c("black","blue"),legend=c("Normal BMI","BMI > 35"), cex=0.9, inset=0.05)
...
legend("right",lty=1,col=c("black","blue"),legend=c("w/o Diabetes","w/ Diabetes"), cex=0.9, inset=0.05)
title(main="c", font.main=4, cex.main=1.2, adj=0)
dev.copy(png,file="Plotm11t40_noother.png",width = 320, height = 800); dev.off()



# final for 3 by 1 at 40
library(sitar)
setwd("~/Projects/R_lib/bmi/data")
load('m11f.Rda')
load('nwaz.Rda')
...
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.1018024530,b=0.3109147150,c=-0.1734587254))
...
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.1261866877,b=0.4728156052,c=-0.3030369190))
legend("right",lty=1,col=c("black","blue"),legend=c("w/o Diabetes","w/ Diabetes"), cex=0.9, inset=0.05)
title(main="c", font.main=4, cex.main=1.2, adj=0)
dev.copy(png,file="Plotm11t40.png",width = 320, height = 800); dev.off()


# final for 2 by 1 at 40

par(mfrow = c(2, 1), oma = c(0, 0, 0, 0), mar = c(4, 4, 2, 4), cex=0.7)
...
dev.copy(png,file="Plotm11t40_2_1.png",width = 320, height = 560); dev.off()

# final for 2 by 1 at 40 - with reduced margin

par(mfrow = c(2, 1), oma = c(0, 0, 0, 0), mar = c(3, 3, 2, 3), cex=0.7)

plot(m11f, opt='u', col=1+as.integer(id) %% 5, yfun=exp,
     xlab="", ylab="", mar = c(4, 4, 2, 5))
mtext("Age in Months", side=1, line=2, cex=0.7)
mtext("Weight (kg)", side=2, line=2, cex=0.7)
title(main="a", font.main=4, cex.main=1.2, adj=0)


plot(m11f, y2par=list(col='black'), yfun=exp, xlab="", ylab="Weight (kg)", apv=T, mar = c(3, 3, 2, 3))
plot(m11f, y2par=list(col='black'), yfun=exp, xlab="", ylab="", apv=T, add=T)
plot(m11f, y2par=list(col='blue'), col='blue', yfun=exp, xlab="", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.1018024530,b=0.3109147150,c=-0.1734587254))
legend("right",lty=1,col=c("black","blue"),legend=c("Normal BMI","BMI > 40"), cex=0.9, inset=0.05)
mtext("Age in Months", side=1, line=2, cex=0.7)
mtext("Weight (kg)", side=2, line=2, cex=0.7)
mtext("Weight (kg) Velocity", side=4, line=2, cex=0.7)
title(main="b", font.main=4, cex.main=1.2, adj=0)
...
plot(m21, y2par=list(col='black'), yfun=exp, xlab="", ylab="Weight (kg)", apv=T, mar = c(3, 3, 2, 3))
plot(m21, y2par=list(col='black'), yfun=exp, xlab="", ylab="", apv=T, add=T)
plot(m21, y2par=list(col='red'), col='red', yfun=exp, xlab="", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=-0.017523766,b=-0.023842045,c=0.039002980))
plot(m21, y2par=list(col='blue'), col='blue', yfun=exp, xlab="", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=0.054474945,b=0.062496335,c=-0.067801963))
...
dev.copy(png,file="Plotm11t40_2_1_150201.png",width = 320, height = 560); dev.off()


# final for 1 by 1 at 40 - with reduced margin
library(sitar)
setwd("~/Projects/R_lib/bmi/data")
load('m21.Rda')
load('nwaz.Rda')

par(mfrow = c(1, 1), oma = c(0, 0, 0, 0), mar = c(3, 3, 2, 3), cex=0.7)
...
dev.copy(png,file="Plotm11t40_1_1_wg_150201.png",width = 320, height = 320); dev.off()

# final for BMI 1 by 1 at 40 - with reduced margin
par(mfrow = c(1, 1), oma = c(0, 0, 0, 0), mar = c(3, 3, 2, 3), cex=0.7)
...
dev.copy(png,file="Plotm11t40_1_1_BMI_150201.png",width = 320, height = 320); dev.off()
