...
waz <- subset(waz, select=c(weight, agemos, id, ref_date, wt_source, sex, BMI_LMP_kgm2, mat_gest_wt_gain_category2,
...
nwaz$weight <- nwaz$height
nwaz$weight <- log(nwaz$weight)
nwaz$weight <- log(nwaz$height)
...
m11a.coef <- data.frame(m11a$coefficients$fixed)
m11b.coef <- data.frame(m11b$coefficients$fixed)
m11c.coef <- data.frame(m11c$coefficients$fixed)

m12a.coef <- data.frame(m12a$coefficients$fixed)
m12b.coef <- data.frame(m12b$coefficients$fixed)
m12c.coef <- data.frame(m12c$coefficients$fixed)

m21a.coef <- data.frame(m21a$coefficients$fixed)
m21b.coef <- data.frame(m21b$coefficients$fixed)
m21c.coef <- data.frame(m21c$coefficients$fixed)

m22a.coef <- data.frame(m22a$coefficients$fixed)
m22b.coef <- data.frame(m22b$coefficients$fixed)
m22c.coef <- data.frame(m22c$coefficients$fixed)
...
# m11abc <- sitar(x=agemos, y=weight, id=id, data=na.omit(nwaz), df=3, a.formula=~sex+BMI_LMP_kgm2+gest_weight_gain_kg, 
