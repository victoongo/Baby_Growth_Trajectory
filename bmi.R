...
ggplot(pwaz,aes(x=agemos,y=height,color=as.factor(id)),alpha=I(1/3)) + 
  geom_path() +
...
pwaz_sex <- nwaz
pwaz_sex$pred <- predict(qm_sex, newdata=pwaz_sex,level=0)
...
ggplot(pwaz_sex,aes(x=agemos,y=height,color=factor(sex)), alpha=I(1/3)) + 
  geom_path() +
