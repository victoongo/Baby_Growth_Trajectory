...
cvar <- subset(nwaz, select=c(GestAge_TotalDays, mom_age_delv, weight, gest_weight_gain_kg))
tblFun <- function(x){
    tbl <- table(x)
...
```{r, echo=FALSE, fig.show='hold'}
plot(m21, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T)
plot(m21, y2par=list(col='black'), yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T, add=T)
plot(m21, y2par=list(col='red'), col='red', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
     add=T, abc=c(a=-0.017523766,b=-0.023842045,c=0.039002980))
plot(m21, y2par=list(col='blue'), col='blue', yfun=exp, xlab="Age in Months", ylab="Weight (kg)", apv=T,
